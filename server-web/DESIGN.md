...
│   └── Rounds/                   # 当前轮次页（Rounds.tsx：进行中轮卡片 13 项指标动态计算、买卖录入——按日期自动匹配确认净值/份额与回款自动带出、交易表自适应填满剩余视口高度可删误录、持有份额未归 0 时闭轮禁用）+ 已清仓轮次页（ClosedRounds.tsx：清仓快照表 + 展开交易明细）；useFundSelection.ts 共享基金选择
