...
│   └── Rounds/                   # 当前轮次页（Rounds.tsx：进行中轮卡片 14 项指标动态计算（含浮动盈亏率/轮总盈亏率独立项；每项 label 带 ⓘ 公式气泡）、买卖录入——按日期自动匹配确认净值/份额与回款自动带出、买入行内「卖出」按钮发起显式配对（份额锁定为该批次剩余份额，表单显示配对 chip 可 ✕ 取消回自动 FIFO）、交易表自适应填满剩余视口高度可删误录（买入行标注剩余/已配对卖出/已清仓，卖出行标注配对买入日期，删除失败展示服务端错误；表上方过滤切换：全部/只看买入/只看卖出/未匹配买入——未匹配买入=仍有剩余份额的买入批次，含部分卖出后的剩余，过滤为空时显示空态提示）、持有份额未归 0 时闭轮禁用）+ 已清仓轮次页（ClosedRounds.tsx：清仓快照表——表头带 ⓘ 公式气泡、轮总盈亏率独立列 + 展开交易明细）；useFundSelection.ts 共享基金选择
