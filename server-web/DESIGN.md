...
├── App.tsx                       # 外壳：Layout + 侧边栏页签（服务状态 / 买卖分析 / 过程回放 / 基金搜索 / 关注列表）+ 心跳轮询（10s，结果显示于顶栏）
├── components/
...
│   ├── FundDetail/               # 基金净值详情页（关注列表点击进入，非侧边栏页签）：净值历史表（日期/单位净值倒序，客户端分页每页 50 条，分页栏固定底部 + 页码跳转）+ 手动补录表单（日期+单位净值；已存在日期提示不覆盖）+ 返回按钮
│   └── Rounds/                   # 轮次页：基金选择（关注列表）+ 进行中轮卡片（13 项指标动态计算，最新净值取 fund_nav）+ 买卖录入表单（选定时间自动匹配当日单位净值为确认净值可手改、无记录日期提示；份额按 金额÷净值 自动带出可手改）+ 本轮交易表（可删误录）+ 历史轮次表（清仓快照，可展开交易明细）；持有份额未归 0 时闭轮按钮禁用
