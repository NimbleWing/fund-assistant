...
│   └── MetricTip.tsx             # 指标公式提示：label 后 ⓘ 图标点击切换公式气泡（再点/点外部关闭），公式文案集中在 lib/formulas.ts（口径对齐 domain_models/CONTEXT.md）
├── features/
...
│   ├── FundDetail/               # 基金净值详情页（关注列表点击进入，非侧边栏页签）：净值历史表（日期/单位净值/涨跌幅/预估净值/预估涨跌幅——实际净值与服务端 16:00 后固化的盘中预估按日期合并展示，两者都有时标注预估偏差%，仅预估无实际的日期也成行；涨跌幅前端按前一实际净值交易日计算（最早行/仅预估行显示 —），预估涨跌幅直接用固化 estimatedPct（相对昨收），红涨绿跌；倒序，客户端分页每页 50 条，分页栏固定底部 + 页码跳转）+ 手动补录表单（日期+单位净值；已存在日期提示不覆盖）+ 返回按钮
│   └── Rounds/                   # 当前轮次页（Rounds.tsx：进行中轮卡片 15 项指标分三张归类卡片动态计算——盈亏总览（轮总盈亏大字号突出 + 轮总盈亏率/已实现盈亏/浮动盈亏（率）/持仓收益·摊薄（对账））、当前持仓（持仓市值/持有本金/持有份额/满30天份额/最新净值）、资金流水（总投入/累计回款/已卖本金/买卖次数），含满30天份额——真实持有期口径，每项 label 带 ⓘ 公式气泡、买卖录入——按日期自动匹配确认净值/份额与回款自动带出、买入行内「卖出」按钮发起显式配对（份额锁定为该批次剩余份额，表单显示配对 chip 可 ✕ 取消回自动 FIFO）、交易表自适应填满剩余视口高度可删误录（买入行标注剩余/已配对卖出/已清仓，卖出行标注配对买入日期；买入行「盈亏」列按批次归因——未卖出显示浮动（最新净值）、已清仓显示已实现（卖出净值）、部分卖出显示合计并标注「已卖 · 持有」构成，列头带 ⓘ 公式气泡，删除失败展示服务端错误；表上方过滤切换：全部/只看买入/只看卖出/未匹配买入——未匹配买入=仍有剩余份额的买入批次，含部分卖出后的剩余，过滤为空时显示空态提示）、持有份额未归 0 时闭轮禁用）+ 已清仓轮次页（ClosedRounds.tsx：清仓快照表——表头带 ⓘ 公式气泡、轮总盈亏率独立列 + 展开交易明细）；useFundSelection.ts 共享基金选择
├── lib/api.ts                    # API 客户端（超时归一，fetch 走 /api 同源/代理）
├── lib/formulas.ts               # 轮指标口径公式文案（METRIC_FORMULAS，ⓘ 气泡内容）
