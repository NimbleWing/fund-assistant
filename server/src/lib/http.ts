// HTTP 基础设施：JSON 响应 / 请求体读取 / 行为化错误 / 安全文件流 / 路由类型。
 ...
 * 必须挂 error 处理——ReadStream 的读错误若无监听器会被 Node 当作 uncaughtException 杀死进程。
 * 客户端中断（断开连接）时同步销毁流，避免后台读完整文件的句柄泄漏。
 */
...
/** 路由请求上下文：params 为路径占位符（如 /api/funds/:code 的 code）。 */
