// 本地服务控制：经 native messaging 引导启动（需先运行 server/install-native.bat 安装 host）。
...
/** native 启动服务并轮询确认上线（每秒一次，最多 10 次）。 */
export async function startServer(
  deps: { send?: SendNative; check?: () => Promise<HealthResult>; wait?: (ms: number) => Promise<void> } = {},
): Promise<StartResult> {
  const send = deps.send ?? sendViaChrome;
  const check = deps.check ?? (() => checkHealth());
  const wait = deps.wait ?? ((ms: number) => new Promise<void>((resolve) => setTimeout(resolve, ms)));

  const r = await send(NATIVE_HOST, { cmd: 'start' });
